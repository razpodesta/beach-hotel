 ...
 * @version 1.0 - Identity Provisioning Standard
...
import { registerCredentialsSchema, type RegisterCredentials, type IdentityDictionary } from '../../schemas/auth.schema';
...
export function RegisterForm({ dictionary, onSubmit, isLoading = false }: RegisterFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 animate-in fade-in duration-700">
      ...
      <div className="space-y-3 pt-2">
        <label className="group flex items-start gap-3 cursor-pointer">
          <div className="relative mt-0.5">
            <input {...register('tosConsent')} type="checkbox" className="peer sr-only" />
        ...
        <label className="group flex items-start gap-3 cursor-pointer">
          <div className="relative mt-0.5">
            <input {...register('newsletterOptIn')} type="checkbox" className="peer sr-only" />
            <div className="h-5 w-5 rounded-lg border border-border bg-background/40 transition-all peer-checked:bg-primary peer-checked:border-primary" />
            <CheckCircle2 size={12} className="absolute left-1 top-1 text-white opacity-0 peer-checked:opacity-100 transition-opacity" />
          </div>
          <span className="text-[10px] text-muted-foreground leading-tight group-hover:text-foreground transition-colors">
      ...
      <button 
        ...
        className={cn(
          ...
          isLoading ? "bg-surface text-muted-foreground" : "bg-foreground text-background hover:bg-primary hover:text-white shadow-primary/10"
        )}
      >
