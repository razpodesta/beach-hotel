...
'use client'; // Marcado como client por Framer Motion
...
// NOTA: Para metadata dinámica, recuerda que en App Router los Server Components
// manejan la metadata. Aquí hacemos una versión simplificada.
export default async function QuienesSomosPage({ params }: { params: Promise<{ lang: Locale }> }) {
  const { lang } = await params;
  const dict = await getDictionary(lang);
  const t = dict.quienes_somos; // Asegúrate de actualizar tu JSON
    ...
    <main className="min-h-screen bg-[#050505] text-white pt-32 pb-20">
      <div className="container mx-auto px-6">
        ...
        <section className="mb-24 text-center">
          <BlurText 
            text={t.hero_title} 
            className="font-display text-5xl md:text-8xl font-bold tracking-tighter justify-center mb-6 text-white" 
          />
          <FadeIn delay={0.3}>
            <p className="text-zinc-400 text-lg md:text-xl max-w-2xl mx-auto italic">
              {t.hero_subtitle}
            </p>
          </FadeIn>
        </section>
        ...
        <section className="text-center bg-white/5 p-16 rounded-3xl border border-white/5">
          ...
          <Link 
            href={`/${lang}/#reservas`} 
            className="rounded-full bg-white px-10 py-5 font-bold text-black hover:scale-105 transition-transform"
          >
            {t.cta_button}
          </Link>
        </section>
