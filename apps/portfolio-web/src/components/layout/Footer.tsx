/**
 * @file Footer.tsx
 * @description Pie de Página Institucional del Ecosistema. 
 *              Refactorizado: Resolución de error TS2339 mediante segregación de 
 *              responsabilidades (SRP). Delega la captação al NewsletterForm.
 * @version 15.0 - Congruent Data Flow & Tailwind v4
 ...
 * @description Contrato de propiedades nivelado con la Constitución de Datos v16.0.
 */
interface FooterProps {
  /** Fragmento del diccionario footer (Navegación e Infraestructura) */
  content: Dictionary['footer'];
  /** Fragmento del diccionario newsletter_form (Captación) */
  newsletterContent: Dictionary['newsletter_form'];
  /** Etiquetas de navegación compartidas SSoT */
  navLabels: Dictionary['nav-links']['nav_links'];
  /** Eslogan de marca inyectado desde el orquestador */
  tagline: string;
  className?: string;
}
...
/**
 * APARATO: Footer
 * @description Cierre institucional. Reacciona dinámicamente a la atmósfera global.
 */
export function Footer({ 
   ...
   */
  useEffect(() => {
    console.log(`[HEIMDALL][UX] Footer synchronized with atmosphere: ${currentLang}`);
  }, [currentLang]);

  /**
   * MAPA DE NAVEGACIÓN EXTENDIDA (MACS Compliance)
   */
      ...
      <div className="container mx-auto px-6 py-24 lg:py-32">
        ...
        <div className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <motion.div 
            ...
            className="rounded-5xl bg-background/40 border border-border/50 backdrop-blur-xl shadow-2xl p-2 transition-all"
          >
            {/* Inyección del aparato de leads con su propio contrato de datos */}
        ...
        {/* --- 3. BARRA DE CUMPLIMIENTO Y AUTORÍA --- */}
        <motion.div 
          initial={{ opacity: 0 }} 
